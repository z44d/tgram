...
- **`media`** (**`Path, str`**): **File to send. Pass a file_id to send a file that exists on the Telegram servers (recommended), pass an
...
- **`thumbnail`** (**`InputFile, str`**): **Optional. Thumbnail of the file sent; can be ignored if thumbnail generation for the file is supported
...
details.**
- **`caption_entities`** (**List of `tgram.types.MessageEntity`**): **Optional. List of special entities that appear in the caption, which can be specified
