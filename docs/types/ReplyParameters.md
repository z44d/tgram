...
- **`chat_id`** (**`int, str`**): **Optional. If the message to be replied to is from a different chat, unique identifier for the chat or username of the channel (in the format @channelusername)**
...
- **`quote_entities`** (**List of `tgram.types.MessageEntity`**): **Optional. A JSON-serialized list of special entities that appear in the quote. It can be specified instead of quote_parse_mode.**
