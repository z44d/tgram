...
- **`supports_guest_queries`** (**`bool`** ): **Optional. True, if the bot supports guest queries from chats it is not a member of.
Returned only in getMe.**
- **`can_manage_bots`** (**`bool`** ): **Optional. True, if the bot can manage bots. Returned only in getMe.**
- **`supports_join_request_queries`** (**`bool`** ): **Optional. True, if the bot supports join request queries. Returned only in
getMe.**
