...
- **`photo`** (**List of `tgram.types.PhotoSize`**): **Photo that will be displayed in the game message in chats.**
...
- **`text_entities`** (**List of `tgram.types.MessageEntity`**): **Optional. Special entities that appear in text, such as usernames, URLs, bot commands, etc.**
