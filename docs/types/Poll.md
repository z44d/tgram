...
- **`correct_option_ids`** (**List of `int`** ): **Optional. 0-based identifiers of the correct answer options. Available only for polls in the quiz mode, which are closed, or was sent (not forwarded) by the bot or to the private chat with the bot.**
...
- **`allows_revoting`** (**`bool`** ): **Optional. True, if the poll allows revoting.**
- **`description`** (**`String`** ): **Optional. Poll description, 0-512 characters.**
- **`description_entities`** (**List of [MessageEntity](MessageEntity.md)** ): **Optional. Special entities that appear in the description. Currently, only custom emoji entities are allowed in poll descriptions.**
- **`media`** (**[PollMedia](PollMedia.md)** ): **Optional. Media in the poll**
- **`explanation_media`** (**[PollMedia](PollMedia.md)** ): **Optional. Media in the quiz explanation**
- **`members_only`** (**`bool`** ): **Optional. True, if only members of the chat can vote in the poll**
- **`country_codes`** (**List of `str`** ): **Optional. List of country codes, for polls that are only available in certain countries**
