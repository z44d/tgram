...
- **`can_react_to_messages`** (**`bool`** ): **True, if the user is allowed to react to messages**
...
- **`can_edit_tag`** (**`bool`** ): **True, if the user is allowed to edit their profile tag**
- **`until_date`** (**`int`** ): **Date when restrictions will be lifted for this user; unix time. If 0, then the user is restricted
forever**
- **`tag`** (**`str`** ): **Optional. User's profile tag in the chat**
