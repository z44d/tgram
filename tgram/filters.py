...
business_connection_id = Filter(lambda _, m: bool(getattr(m, "business_connection_id", False)))
...
protected_content = Filter(lambda _, m: bool(getattr(m, "has_protected_content", False)))
...
supergroup_chat_created = Filter(lambda _, m: bool(getattr(m, "supergroup_chat_created", False)))
channel_chat_created = Filter(lambda _, m: bool(getattr(m, "channel_chat_created", False)))
message_auto_delete_timer_changed = Filter(lambda _, m: bool(getattr(m, "message_auto_delete_timer_changed", False)))
migrate_to_chat_id = Filter(lambda _, m: bool(getattr(m, "migrate_to_chat_id", False)))
migrate_from_chat_id = Filter(lambda _, m: bool(getattr(m, "migrate_from_chat_id", False)))
...
write_access_allowed = Filter(lambda _, m: bool(getattr(m, "write_access_allowed", False)))
passport_data = Filter(lambda _, m: bool(getattr(m, "passport_data", False)))
proximity_alert_triggered = Filter(lambda _, m: bool(getattr(m, "proximity_alert_triggered", False)))
boost_added = Filter(lambda _, m: bool(getattr(m, "boost_added", False)))
chat_background_set = Filter(lambda _, m: bool(getattr(m, "chat_background_set", False)))
forum_topic_created = Filter(lambda _, m: bool(getattr(m, "forum_topic_created", False)))
...
forum_topic_reopened = Filter(lambda _, m: bool(getattr(m, "forum_topic_reopened", False)))
general_forum_topic_hidden = Filter(lambda _, m: bool(getattr(m, "general_forum_topic_hidden", False)))
general_forum_topic_unhidden = Filter(lambda _, m: bool(getattr(m, "general_forum_topic_unhidden", False)))
...
video_chat_scheduled = Filter(lambda _, m: bool(getattr(m, "video_chat_scheduled", False)))
...
video_chat_participants_invited = Filter(lambda _, m: bool(getattr(m, "video_chat_participants_invited", False)))
...
def sender(ids: Union[str, int, List[Union[str, int]]]) -> Filter:
    ...
    ids = {ids.lower() if isinstance(ids, str) else ids} if not isinstance(ids, list) else {i.lower() if isinstance(i, str) else i for i in ids}
    return Filter(lambda _, m: getattr(m, "sender_chat") and (m.sender_chat.id in ids or (m.sender_chat.username and m.sender_chat.username.lower() in ids)))


def user(ids: Union[str, int, List[Union[str, int]]]) -> Filter:
    """Filter messages coming from one or more users"""
    ids = {ids.lower() if isinstance(ids, str) else ids} if not isinstance(ids, list) else {i.lower() if isinstance(i, str) else i for i in ids}
    return Filter(lambda _, m: getattr(m, "from_user") and (m.from_user.id in ids or (m.from_user.username and m.from_user.username.lower() in ids)))


def chat(ids: Union[str, int, List[Union[str, int]]]) -> Filter:
    """Filter messages coming from one or more chats"""
    ids = {ids.lower() if isinstance(ids, str) else ids} if not isinstance(ids, list) else {i.lower() if isinstance(i, str) else i for i in ids}
    return Filter(lambda _, m: getattr(m, "chat") and (m.chat.id in ids or (m.chat.username and m.chat.username.lower() in ids)))
...
def regex(pattern: Union[str, Pattern], flags: int = 0) -> Filter:
    ...
    async def regex_filter(_, m):
        if not isinstance(m, (tgram.types.Message, tgram.types.CallbackQuery, tgram.types.InlineQuery, tgram.types.PreCheckoutQuery)):
            raise ValueError(f"Regex filter doesn't work with {m.__class__.__name__}")

        value = (m.text or m.caption) if isinstance(m, tgram.types.Message) else m.data if isinstance(m, tgram.types.CallbackQuery) else m.query if isinstance(m, tgram.types.InlineQuery) else m.invoice_payload if isinstance(m, tgram.types.PreCheckoutQuery) else None
...
def chat_type(types: Union[list, str]) -> Filter:
    ...
    async def chat_filter(_, m):
        if isinstance(m, tgram.types.CallbackQuery) and m.message and m.message.chat:
            ...
            raise ValueError(f"Chat type filter doesn't work with {m.__class__.__name__}")
...
def command(commands: Union[str, List[str]], prefixes: Union[str, List[str]] = "/", case_sensitive: bool = False) -> Filter:
    ...
    async def filter_func(b: "tgram.TgBot", m: "tgram.types.Message"):
        ...
        for prefix in prefixes:
            ...
            for cmd in commands:
                if not re.match(rf"^(?:{cmd}(?:@?{username})?)(?:\s|$)", without_prefix, flags=re.IGNORECASE if not case_sensitive else 0):
                    continue

                without_command = re.sub(rf"{cmd}(?:@?{username})?\s?", "", without_prefix, count=1, flags=re.IGNORECASE if not case_sensitive else 0)
                m.command = [cmd] + [re.sub(r"\\([\"'])", r"\1", m.group(2) or m.group(3) or "") for m in command_re.finditer(without_command)]
                return True
